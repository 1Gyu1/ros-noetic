...
bool QNode::init() {
	ros::init(init_argc,init_argv,"controll_robots");
	if ( ! ros::master::check() ) {
		return false;
	}
	...
	ros::NodeHandle n;
    rb_command_pub = n.advertise<controll_robots::rb_command>("/rb_command", 1);
...
void QNode::rb_data_callback(const controll_robots::rb_data::ConstPtr &msg){
    if(ignore_cnt > 0){
        ignore_cnt--;
...
bool QNode::init(const std::string &master_url, const std::string &host_url) {
	...
	ros::init(remappings,"controll_robots");
	if ( ! ros::master::check() ) {
		return false;
	}
	ros::start(); // explicitly needed since our nodehandle is going out of scope.
