...
def build_chain():
    ...
    # test model
    # trans[0] = [0.0, 0.0, 0.197]
    # trans[1] = [0.0, 0.0, 0.6127]
    # trans[2] = [0.0, 0.0, 0.57015]
    # trans[3] = [0.0, -0.15625, 0.0]
    # trans[4] = [0.0, 0.0, 0.11715]
    # trans[5] = [0.0, -0.1153, 0.0]
    while True:
        try:
...
class TaskController_RB(Robot):
    ...
    def broadcast_tf_test(self, _frame):
        tcp_trans = _frame.p
        tcp_rot = _frame.M.GetQuaternion()
        br = tf.TransformBroadcaster()
        br.sendTransform(
            (tcp_trans[0], tcp_trans[1], tcp_trans[2]), 
            (tcp_rot[0], tcp_rot[1], tcp_rot[2], tcp_rot[3]), 
            rospy.Time.now(), "goal", 'Axis_0') 
    ...
    def traj_move(self, traj, traj_len):
        ...
        while not self.isstop:
            ...
            if self.ROBOT == self.READY_STATE:
                ...
                # self.broadcast_tf_test(goal_frame)
    ...
    def pick_and_place_demo(self):
        ...
        fr = super().get_frame_XYZ(fr, _x=0.1, _y=0.4)
        self.waypoint[6] = [fr, 'j']     # mid point

        ################ place ################
        fr = super().get_frame_XYZ(fr, _x=-0.1, _y=0.65, _z=-0.3)
    ...
    def convert_tcp(self):
        ...
        Returns:
            tcp: real tcp frame
        """
        # print("test sign data: ",self.go_sign)
        super().update_joint(self.jointAngles)
    ...
    def status_cb(self, status):
        ...
        # 2 = Pause
        # 3 = Moving
        self.rb10_data.robot_status = self.ready = status.robot_state
    ...
    def joint_cb(self, jnt):
        fr = self.convert_tcp()
        for i in range(6):
            self.rb10_data.joint[i] = self.jointAngles[i] = jnt.position[i]
        ...
        for idx, val in enumerate(trans):
            self.rb10_data.tcp[idx] = val
        for idx, val in enumerate(rot, start=3):
            self.rb10_data.tcp[idx] = math.degrees(val)
    ...
    def msg_cb(self, cmd_msg):
        ...
        #     self.isstop = True
        #     self.stop_pub.publish(True)
        #     print("Test loop signal: ",self.isstop)
        #     # self.isstop = False
        # elif self.go_sign.data == "q":
        #     self.isshutdown = True
        #     self.node_shutdown_pub.publish(True)
...
def main():
    rospy.init_node("task_controller_rb10")
    rb10 = TaskController_RB(build_chain())
